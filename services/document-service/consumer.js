...
const queueName = 'document-manager'; // Bu servisin dinleyeceği kuyruk
...
amqp.connect(rabbitmqUrl, (err, connection) => {
  ...
  connection.createChannel((err, channel) => {
    ...
    console.log(`[*] document-manager servisi ${queueName} kuyruğunu dinliyor...`);

    // Mesajları tüketme
    channel.consume(queueName, (msg) => {
      if (msg !== null) {
        const messageContent = msg.content.toString();
        console.log(`[x] document-manager Mesaj alındı: ${messageContent}`);
