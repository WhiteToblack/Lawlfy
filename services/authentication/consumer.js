...
const queueName = 'authentication'; // Bu servisin dinleyeceği kuyruk
...
amqp.connect(rabbitmqUrl, (err, connection) => {
  ...
  connection.createChannel((err, channel) => {
    ...
    console.log(`[*] authentication servisi ${queueName} kuyruğunu dinliyor...`);

    // Mesajları tüketme
    channel.consume(queueName, (msg) => {
      if (msg !== null) {
        const messageContent = msg.content.toString();
        console.log(`[x] authentication Mesaj alındı: ${messageContent}`);
